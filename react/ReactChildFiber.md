...
function ChildReconciler(shouldTrackSideEffects) {
  ...
  function reconcileChildrenArray(
    ...
    // 新的 children 长度已经够了，删除多余的节点
    if (newIdx === newChildren.length) {
      deleteRemainingChildren(returnFiber, oldFiber);
      return resultingFirstChild;
    }

    // 如果老的 children 遍历玩了，需要创建剩余的节点
    if (oldFiber === null) {
  ...
  function reconcileSingleTextNode(
    ...
    // 如果是更新，且已经有 text 节点，那么可以直接复用节点并更新内容
    if (currentFirstChild !== null && currentFirstChild.tag === HostText) {
      deleteRemainingChildren(returnFiber, currentFirstChild.sibling); // 删除后面多余的节点
      const existing = useFiber(currentFirstChild, textContent, expirationTime);
      existing.return = returnFiber;
      return existing;
    }
    // 如果不是 text 节点 则从头删除后重新创建
  ...
  function reconcileChildFibers(
    returnFiber: Fiber, // 执行 diff 的父节点
    currentFirstChild: Fiber | null, // 当前第一个 fiber
    newChild: any, // 要更新的进节点的 children 数据
    expirationTime: ExpirationTime
  ): Fiber | null {
