...
1. 当 tag 为 `FunctionComponent`，会调用 `commitHookEffectListUnmount`。该方法会遍历 `updateQueue`，同步执行所有 `useLayoutEffect` 的销毁函数。
...
2. 对于 `FunctionComponent` 及相关类型，他会同步调用 `useLayoutEffect` 的回调函数。
