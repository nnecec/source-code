...
2. 然后获取该 fiber 的兄弟 DOM 节点
3. 根据 DOM 兄弟节点是否存在决定调用 `parentNode.insertBefore` 或 `parentNode.appendChild` 执行 DOM 插入操作。

`commitWork`方法

1. 当 tag 为 `FunctionComponent`，会调用 `commitHookEffectListUnmount`。该方法会遍历 `updateQueue`，执行所有 `useLayoutEffect` 的销毁函数。
1. 当 tag 为 `HostComponent`，会调用`commitUpdate`(ReactDOMHostConfig)。最终会在 `updateDOMProperties` 中将 `render` 阶段 `completeWork`中为 fiber 节点赋值的 `updateQueue` 对应的内容渲染在页面上。
...
1. 对于`classComponent`，调用`componentDidMount/componentDidUpdate`。
2. 对于 `FunctionComponent` 及相关类型，他会调用 `useLayoutEffect` 的回调函数，调度 `useEffect` 的销毁与回调函数
   结合这里我们可以发现，`useLayoutEffect` 从上一次更新的销毁函数调用到本次更新的回调函数调用是同步执行的。
